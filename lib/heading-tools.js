...
function bookmarkButton({ anchor, title, pageUrl }) {
  return [
    ...
    ` data-bookmark-url="${escapeAttr(`${pageUrl}#${anchor}`)}"`,
    ` aria-pressed="false"`,
...
export function enhanceHeadings(content, options = {}) {
  ...
  const { bookmarkable = false, pageUrl = "", anchorAliases = [] } = options;
  ...
  return html.replace(H2_RE, (_match, anchor, attrs, inner) => {
    ...
    const tools = [
      ...
      bookmarkable ? bookmarkButton({ anchor, title, pageUrl }) : "",
    ].join("");
