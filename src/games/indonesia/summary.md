...
The player with the most {% term "rupiah" %} at the end of the game wins (Bank + Cash).
...
The game end trigger happens when one or zero c {% term "era" %} companies are left to be acquired in the {% rule "New Era" %} phase.
...
At the start of an era players place cities in turn order based on their city building cards for that era. They may choose one region on that card to place one green city. The city must be placed in an empty area on a coast. Each region can only contain one city.
...
Players must have researched at least one {% term "merger" %} on the {% rule "R&D track", "research-and-development" %}.

A merger is only between 2 companies.

Companies can only be involved in a merger once per year.

Mergers always happen between companies of the same type with the exception of {% term "siap faji", "siap-faji" %} mergers that can happen between a rice and a spice company. 2 Siap Faji companies may also merge. Siap Faji mergers cannot occur in Era A, only B and C.
...
The bid must be at least equal to the {% term "nominal value", "nominal-value" %} of the merged companies (number of goods/ships × value on player aid) OR a valid raised bid (bids must be divisible by the total number of goods/ships involved).
...
In turn order players may start one new company per round provided they have an open {% term "slot" %}.
...
Number of companies a player may own. Merged companies take up one {% term "slot" %}.
...
Number of goods each ship may carry for that player — see {% term "hull" %}.
...
If a merger has caused a company to have 2 {% term "production zones", "production-zone" %}, each needs its own shipping.
...
## Victory & End Game {: #victory-and-end-game}
