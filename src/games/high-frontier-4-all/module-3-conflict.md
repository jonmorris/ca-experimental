...
## 3A. Module 3 {: #3a-module-3}

As Factions expand into the solar system and trade among themselves, they become less dependent on Earth and Tip (Modules 1 & 2): In a less tolerant of Earth-centered trade monopolies and regulations. Secessionists start to clamor for Independence Futures game with Module and self-governance. But if the War of Independence (War) is declared, the game state changes drastically until 3, you can no longer safely it is resolved, with hostilities between the Loyalists and the Independents . At stake: will space ventures and ignore the political arena

just because it is over-

pioneers be subject to regulatory governance by terrestrial superpowers and multinational organizations like the

shadowed by other sources

UN, or will they become a free trade coalition of self-governing city-states?[1] of VP. 3Aa

**a. Modules.** Module 0 must be used, using the Module 3 Political Assembly placard (instead of the Core placard). Modules 1 & 2 are recommended (but not required). All Module 1 & 2 rules are listed in blue font. Easily missed (Module 2): You can also play with *Futures* (**1D**), which can influence combat (**3F2**).Both your *faction privileges*

**b. Number of Players.** 2 to 6 competitive. Solitaire is not supported.(B6a) and *faction weapons*

**c. Piracy Variant.** Combat is allowed but not mandatory at all times against all opponents, but if your victim is (3D6) are shutdown unless in the same *bloc* as you, you must pay him a *bribe*, following the *vendetta* rules (**3D1**).[2] you have a Home Bernal

**d. Age of Piracy Variant.** Players who want to experience combat without the War of Independence can play (either per 2B3 or 3F1). 3Aa the Piracy Variant as above, but without blocs, propaganda, or extremists, and all attacks require paying the

Easily missed: If using

defender a bribe. Do not use the **3A2f,g** *setup*, *Module 3 political assembly* (**3B2**), *propaganda* (**3B3**), *war*

Module 1, the *delivery*

*outbreak* (**3C**), *ET Home Bernal & Postwar* (**3F**), and *War/Anarchy* (**3D,** except *faction weapons* **3D6,** *dislodge*

*operation* (I9) can’t be used,

**3D7** and *combat* **3D8** are used).

see 1A8. 3Aa

**e. Modified Future Wars Variant.** A game without combat can use the *evade* rules (**3D3, 4, 5**) to modify Wars of Independence that happen as a result of *casus belli* Futures (**1D2c**). As there are no *extremist* or *corrupt* Tip: Age of Piracy is a good *delegates* **(3D2)**, only your own delegates or allied delegates with permission can be petitioned to evade.way to try out the combat

**f. Software Versions.** Tabletop Simulator by Justin Grey, Vassal by Stefano Tiné.rules, or if you are playing a

shorter game (e.g. only 48 years or without Modules 1

& 2). 3Ad

#### Notes

[1] Cover art by Josefin Strand depicts an attack on a cometary mining factory using a laser-heated missile with a beamed gray goo warhead. In general, facilities on comets and asteroids are hardened targets, with their facilities either mobile or deep underground. Although this is a violation of exoplanetary law, they can be infected with nanites capable of replicating using sunlight and regolith. Another attack strategy takes advantage of the fact that many comets are covered with low density amorphous water-ice (Asw), a type of ice stable in the outer Solar System. If a chain reaction is initiated, runaway recrystallization of this ice can cause it to explode.

[2] **Blood Credits.** The bribe gained by a pirate’s victim simulates, not a share of the booty, but rather a blood bribe that gives the victim a justifiable cause for revenge or justice. This is not just talk, such eye-for-an-eye justice is respected by others considering joining the victim's cause.

### 3A1. Module 3 Components

**1 Drawer Box. 2 Rulebooks.** These rules plus a Strategy Guide. **6 Playmats.** 1 per player. **2 Player Aids. 1 Module 3 Political Assembly Placard. 12 Bribe Disks.** Black, see **3B1. 6 Dropstone Tokens.** Place on the map if a dropstone is *dislodged* **(3D7)** to show the location of its stack. **6 Propaganda Chits.** Setup per **3A2f.** Flipped per **3B3a,b,c.** Determines victory in War per **3F4. 11 Damage Chits.** Indicators for Factory damage **(3E7c). 1 Red Cross Chit.** Indicator for *cancer hospital* **(3E9d). 1 Lutetia Site Chit.** (optional Punch out). **5 Pow Chits.** Special glory chits **(3D9c). 1 Six-sided dice.** You will also need one six-sided dice from the Core Game. **3 Ziplock Bags**
...
**b. Assign Factions & Crew.** Players divide themselves into **Independence** and **Loyalist** blocs, then each bloc among themselves assigns factions to each player (use dice if necessary). There must be at least one Loyalist player [Purple (socialist), Yellow (nationalist), or White (globalist)] plus one Independence player [Green (unionist), Gray (individualist), or Red (entrepreneur)].
...
**f. Propaganda Chits** are placed on the circles on the political assembly placard. Freedom, individuality, and equality are placed Independence bloc up, and honor, unity, and authority are placed Loyalist bloc up, as shown. No propaganda chit is used for centrist.[3]

**g. Extremist Delegates.** Each player starts with one delegate placed in his color of Ideology. Unlike Core, place your delegate on top of the propaganda chit that sits in the color of your Ideology. As long as it remains on the propaganda, this is your **extremist delegate (3B2a).**[4] Optional: The latest findings from the Rosetta spacecraft found no hydrated minerals on the asteroid Lutetia ( 8:30), contrary to 2004 data. We have provided a hex-shaped chit that you may place on the map to use the latest data. 3A1
...
Six propaganda chit setup 3A2f
...
**c. Defection (flips your own extremist)**. As a free action on your Turn, you may flip your extremist propaganda. Tip: In a 2-player game, This **disgraces** your delegate (i.e. permanently moves it off the chit and into the Ideology). Furthermore, you can always end the War if you have uncolonized Factory(s), you must choose one to convert to the color of one of your enemies by defecting! You lose a **(3E2k)**. At the end of the War, players without extremists are counted among the *defeated* **(3F4d)**.[12] Factory. 3B3c

- **Surrender.** Defection can also be involuntary, as a result of combat **(3B3a)**. If so, you do not suffer a Factory

Easily missed: You never

loss, but are not allowed *whistleblowing* or *sneak attacks* (see below).

forfeit a Factory unless the

- **Capitulation.** If this results in all propaganda being the same bloc, the War ends (in your defeat!) at the end defection was voluntary. of your Turn, see **3F4**.3B3c
...
Easily missed (Module 2):

The Individuality Law (*Freedom to Roam Treaty* **(O5f)**) is expanded to allow the sale of a Black-Side Card if taken to

The individuality law (O5f)

an allied or enemy Bernal, (but not your own!) with the following conditions:[13] states: "You may treat

**a. No Snow To Inuits.** The Bernal must not be Adjacent to a Dirtside with the same Spectral Type as the Black-an opponent’s Factory Side Card being sold. or Bernal as your own."

This makes your Bernal
...
## 3C. War Of Independence Outbreak {: #3c-war-of-independence-outbreak}
...
**b. Determine Your Bloc.** During the first War/Anarchy, Players Purple, Yellow, and White start as Loyalists (), while Players Green, Gray, and Red start as Independents ().[15] This bloc can change as a result of losing a battle **(3B3a)** or *defection* **(3B3c)**.
...
**Example 3C1** After Turn 36, there are 3 Seniority Disks in the political assembly. You build the first factory on an **M** asteroid. The stock price on the **M** column drops to 8, and you make a war outbreak roll. You roll “6”, plus 3 = 9. War is declared!

Starting bloc 3Cb

Players: Green, Gray and Red

Players: Purple, Yellow, WWhhiittee

**Easily Missed:** The term 'opponent' in these rules is used to define all other players, not allies or enemies. 3Cc

#### Notes

[15] Astrological symbols used for Earth () and Sol () also signify the Loyalist and Independence Blocs respectively. These icons were codified during the Renaissance for zodiac cults, and are still used in the science of astronomy. Using the sun icon as an icon for liberation furthers the heliocentric revolution started by Copernicus, which dethrones the Earth as the center of the universe.
...
**b. Remove Star Token.** Move the Active Law star into the War/Anarchy slot (yellow star) on the political Easily missed: *Suborn* assembly. This indicates Laws cannot be made active or lobbied.(3D2b) is a once-per-turn

**c. Purge Roll (K1e).** The 1st Player rolls one dice (1d6) and consults the dice numbers printed on the assembly. free action, so you can only

save one of your delegates in

Decommission (if possible) one delegate of each player color in this Ideology. Extremists are immune!

enemy ideologies. 3C3d
...
**e. Unanchor Independence Bernals (Module 2).** If any Independence player has a Home Bernal, by the end

If unanchored, do not forget

of his Turn he must either (1) evade the “No Home Bernal Anchor” restriction (with a *petition* in authority), or

dirt-refuel per 3E9a. 3C3e

(2) it is unanchored.[18] Furthermore, a Bernal without any allied Dirtsides (i.e. all of its Dirtside Factory cubes belong to the enemy), is similarly unanchored **(3E9a)**.Tip (Module 2): If you are

**f. Desertion of Enemy Colonists (Module 2).** The *faction Ideology* **(B6b)** of each Colonist determines which Independence and have a bloc the Colonist is allied with (checking the propaganda in its Ideology). If at the end of your Turn you are in Home Bernal, keep a spare one bloc, and a Colonist of yours supports the other bloc, you must either (1) evade the “No Enemy Colonists” generator in your Bernal restriction (**3D5d** honor), or (2) Discard this enemy Colonist to the bottom of the queue.Stack, so that you will be

- **Instances of Desertion.** This can happen during War/Anarchy outbreak **(3C3e),** whenever the Colonist’s able to travel to a better

location in case of War.

*propaganda* is flipped **(3B3a,b,c)**, or if you *defect* **(3B3c).**

Furthermore, if your Colonist

- **Exomigration.** If a Colonist deserts during your Turn, exomigration occurs on your following Turn.

is a Loyalist, remember that they will desert (3C3f).
...
**Example 3D2f** War breaks out and you are Independent. Now you want to boost your Bernal. You will need to evade the War restrictions on boosting, so you spend 4 Aqua to suborn a foreign delegate in the unity Ideology.

**g. Lingering Corruption.** A corrupt delegate remains corrupted after War/Anarchy ends.

Corrupt delegate 3D2
...
**Example 3D3b** Player Yellow fundraises, moving a delegate into honor. Then he makes a negotiation with Player Green for use of her Powersat for movement. The restrictions against negotiating with the enemy are evaded by the yellow delegate in honor.
...
The following Operations are **restricted**, as indicated on the political assembly by the War stop sign **War** . This means you cannot perform them unless you *petition* a delegate to *evade* the restrictions **(3D3)**:
...
**Example 3D4b** It is War, and Player Red wants to boost a card. She has no delegates, but fortunately Player Green has a corrupted delegate in unity that she can use to evade the unity restrictions on boosting.
...
**Example 3D4d** It is War, and Player Gray wants to auction a card. He has a delegate in equality he can use to evade the equality restrictions on auctions. All players can participate in the auction, because only initiation is restricted, not participation.

**e. Limited Promotion Operations (Individuality, extremist = Player Gray).** *Promotions* **(1A5, 2A3)** are not allowed in the (Venus), (Earth), or (Mars) Zones.[23]
...
**Example 3D5c** It’s War, and you are Independence Player Red. You want to boost and humbly ask Player Yellow for permission. Player White interrupts, pointing out that for two enemies to negotiate, they need her permission first. You end up paying her a Hand Card, and Player Yellow a bribe chit, before boosting.
...
- **Synodic Comets.** A dropstone from one of these comets is deorbited into Sol, and on the next Turn can move outward at a rate of one Heliocentric Zone per Turn, starting in the Venus Zone. It can attack when it reaches the targeted Heliocentric Zone per **3D7f**.

- **Jovian Himalia Moonlets.** A dropstone from one of these 4 moons is deorbited into Jupiter, and the Turn afterwards can attack any Site or Anchored Bernal in the Jupiter Zone per **3D7e**.[28]

- **Saturnian Norse Moonlets & Methone**. A dropstone from one of these 6 moons is deorbited into Saturn, and the Turn afterwards can attack any Site or Anchored Bernal in the Saturn Zone.

- **Uranian Setebos/Sycorax/Prospero Moonlets.** A dropstone from one of these moons is deorbited into Uranus, and the Turn afterwards can attack any Site or Anchored Bernal in the Uranus Zone.

- **Frostline Dislodge.** A dropstone dislodged as a tank of water from a Spacecraft can target explosive ice worlds **(3D7g)**, where the impacting water causes a chain reaction.[29] The Spacecraft must be in the Ceres or Jupiter Zones, but not on a Site, and must expend either 5 FT or 5 tanks of water fuel in the dislodge operation.[30] For its first turn it enters the solar oberth flyby **(H8e)**.
...
- **Dropstone From Other Sites.** This dropstone is a boulder, and the dropstone thruster must be a non-Human dirt thruster with a base thrust of 3 or higher.

- **Frostline Dropstone.** This dropstone is a 200 tonne tank of water (5 FTs), and the dropstone thruster must be a non-Human water thruster with a base thrust of 3 or higher. Fuel is not tracked. (The impactor must be water, since only water can set off the runaway crystallization[31] at the target.)
...
**Example 3D7c** You dislodge a dropstone from himalia with a spheromak GW thruster plus its support (an electric generator with a reactor support and remember that the cooling requirements are ignored).
...
- **Combat/Events: Flyby or Space Oberth Flyby.** In one of these Spaces, the dropstone can be range attacked or suffer events as a Site target. The entire Stack is Decommissioned if its thruster becomes non-operational.

**Example 3D7d** The dropstone from the previous example is placed into the Jupiter flyby on its turn of dislodgement. The enemy moves a ship to a radiation belt Adjacent to the flyby and opens fire with two rayguns, taking out the generator. This Decommissions the dropstone.
...
**f. Dropstone Movement/Arrival (Synodic Dropstone 2nd Turn).** A dropstone dislodged from a Synodic Comet must on its 2nd Turn move from the solar oberth ( Zone) into the Venus Heliocentric Zone spot (see illustration in example **3E6**). On each Turn thereafter, the dropstone has only 2 options: (1) Attack any Site or Anchored Bernal in the Zone it occupies, or (2) move to the next Zone spot on the map further from Sol. However, it cannot move to a Heliocentric Zone further from Sol than the Site it was dislodged in.

**g. Dropstone Movement/Arrival (Frostline Dropstones).** A frostline attack uses the same rules as a Synodic Dropstone (previous bullet), except that the target can only be the Tokens (Factory, Colony, Outposts) on a Site that is Hydration 3 or 4 in the Ceres or Jupiter Zones.[33] A Bernal in one of these Zones may also be targeted.
...
**Tip:** For dropstone strategy, locations, and a diagram of a typical dropstone heliocentric orbit, see 3H3. 3D7

Jovian himalia moonlets 3D7b
...
Saturnian norse moonlets 3D7b

Saturnian methone moonlet 3D7b

Uranus sycorax group 3D7b

Dropstone token 3D7c

Dropstone at solar oberth 3D7c
...
**Example 3D8b** You have landed a raygun on an enemy Factory. An enemy Freighter is orbiting overhead, so you have two potential targets. Since the Factory is a pretty tough target, you choose the Freighter as the defending space.
...
**Example 3D8d** In the previous example, the Freighter has a Glitch and so must make a Glitch Roll. This destroys the Freighter, leaving its Cargo as an Outpost.
...
**Example 3D8e** In the previous example, you must target what is left of the Freighter (the Outpost). You cannot switch your attack to the Factory. Since you have a raygun, you fire first, and Decommission the target.
...
**Example 3D8h** In the preceding 4 examples, suppose you had a gun instead of a raygun. The order of combat would be: (1) Choose defending space (the Freighter), (2) Resolve Glitches, (3) Because you do not have jammers or a raygun, the defender fires first. The Freighter has no weapons, but the Factory is allowed to fire even though it was not targeted. (4) Assuming you survive the projectile from the Factory, you roll for your fire against the Freighter.

**i. POWs & Post Battle Decommissions.** Directly after the battle, Human POWs can be claimed by a manned and victorious attacker per **3D9c**. Then both the attacker and defender can Decommission surviving non-Human cards. This can be useful to prevent further *victorious attacks* by the enemy.

Valhalla attacked! 3D7d

**Himalia dropstone on its 2nd Turn.**
...
**Example 3D9a** Your dropstone damages a colonized Yellow Factory and the collateral roll removes 3 Outpost cards. After paying a bribe, you flip the Yellow propaganda, which is the last Loyalist propaganda, ending the War with an Independence victory.
...
|---|---|---|---|---|---|---|
| Raygun (3Ea) | Yes (if non-atmospheric & not connected by buggy road) | Attacker | 2d6 minus ISRU. (3d6 minus ISRU if beamed gray goo). | -2 FV if Site has Adjacent Crash Hazard(s) | | |
| | | | | 6+: | 7+ : | 1+: Zap |
| Gun (3E3b) (cards with RH = 8+) | | Defender | Firing RH minus 2d6. (Factory gun = RH 8). | 1+ : | 4+ : | 1+: Overload, |
| Fighting mirror (needs Powersat) (3E3c) | | Defender | 2d6 minus ISRU. | -2 FV if Site has Adjacent Crash Hazard(s) | | |
| | | | | 7+ : | 8+ : | 1+: Zap |
| Buggy (3E4) | Yes (if connected by | Defender | 2d6 minus ISRU. | 1-2: | 1-4 : | 6+ : |
| Kamikaze (3E5) (needs thruster) | buggy road) No | Defender | Base thrust minus 1d6 (Decommission kamikaze stack). | 3+: Cap | 5+ : +5 FV if biological warhead | |
| | | | | 1+ : | 5+ : | 1+: Overload, |
| Dropstone (3E6) | No | Attacker | { 2d6 | 3-4: | 3-7: | 5+ : |
| Amat bomb (3E3d) | Yes | Defender | | 5+: | 8+: | |

Easily missed: An overload

is not affected by the presence of a Glitch disk, nor does it place a Glitch. Cards with a RH > 6 are immune to **Firing results table legend**

overload. 3E2d**a.**

**b. RH** is short for *rad-hardness* **(B2j)**.

Remember: A Glitch Roll

**c. 2d6** = the sum of the roll of 2 dice.

Decommissions all target cards with the matching rad-hardness. [3E2d, i]

**e. Unanchor** Damage chit 3E2gIf already unanchored, it receives a Glitch disk. **f.** Bernal is Decommissioned, this is *genocide* **(3E2m)**.
...
**.** The targeted Bernal suffers the effects of **(3E9a)**, including moving or removing its dome.

**Zap.** Decommission 1 card, chosen by the firer, with rad-hardness less than the fire value. If an Anchored

**.** If an undamaged Factory is present, place a **damage chit** to show it is *damaged* **(3E7c)**.
...
- **Modifiers.** If the target is a Site (manned or unmanned) with any Adjacent Crash Hazards, subtract 2 from fire value (regardless of the number of Hazards).[40] Your big ship with 2 rayguns moves into the intersection shown outside kleopatra, where an enemy manned rocket belonging to Player Red has just landed. The 2 Hazards do not count towards Adjacency, so you may open fire. Your rayguns allow you the first shots. The first raygun rolls 8 minus its ISRU of 3 and minus 2 more for the Hazard modifier equals 3.

This is not quite high enough since a manned site needs a FV of 4+ to attain a collateral roll combat effect. The second raygun also misses, and then the defender fires their own raygun.
...
**Example 3E3b** Your 2 buggies (1 of which is your Crew) storm an enemy Factory. She fires first with her factory gun, rolling snake eyes (2). Thus her FV is 8 - 2 = 6. For her collateral roll, she rolls a 4. Both of your buggies have RH = 4, and both are Decommissioned.

**c. Fighting Mirror** is any card with an ISRU platform that receives beamed power via the Powersat privilege, [42] which can be negotiated **(N4a)**. A fighting mirror must be in space; it will not work on a Site.

- **Fire Value** is 2d6 minus the mirror's ISRU.

- **Modifiers.** If the target is a Site (manned or unmanned) with any Adjacent Crash Hazards, subtract 2 from fire value (regardless of the number of Hazards). You have a Powersat Home Bernal, which allows your robonaut in the gefion family barycenter to attack Ceres. The Submarine Factory/Colony is underwater and safe from attack, but the enemy has an Outpost there. You make a 1d6 collateral roll = 3. This Decommissions the 2 Outpost cards that have RH = 3.
...
**Yellow dotted line shows path of raygun beam.**

ToughSF, Laser-Coupled Particle Beam
...
**e. Immune to Counterattack.** The dropstone attack is unaffected by combat during the Turn of its attack. Your heliocentric dropstone token is in the Earth Zone as shown. It's your turn, and you can either move it into the Mars Zone, or use it to attack an enemy Bernal at Earth-Luna Lagrange L2. You choose the latter, and pay the defender a bribe. After all your Spacecraft have moved, you roll 2d6 for your attack. Your roll is a 5, enough to unanchor the Bernal.

### 3E7. Factory Damage & Repair

A Factory can suffer 3 possible combat effects: **capture damage** or **destroy .**[46]
...
**f. Mobile Factories.** Promoted Freighters **(1B6)** on a Site defend as Factories. If damaged, they cannot liftoff. If not on a Claim, they defend as a Spacecraft (using the Freighter card’s values including rad-hardness, see **1B6a**). The enemy dislodges a heliocentric dropstone, and you suspect your Factory is at ground zero. You promote your Freighter, allowing you to move the targeted Factory as a Mobile Factory into the safety of space.
...
Dropstone example 3E6

**Tip:** Rayguns are powerful in attack or defense against space targets, but lack the punch necessary against Factories or Colonies. Guns, buggies, dropstones,

biological warheads, or amat bombs are better. 3E7b

Factory damage chit 3E7c
...
- **Homeless (2B6b).** During peace, the number of Colonists you are allowed is equal to your Anchored Bernals **(2Ca)**, so that upon unanchoring you must Discard your choice of one of your Colonists (two, if the Bernal is promoted). During War/Anarchy, the number of Colonists you are allowed is equal to your Bernals

(both Anchored and unanchored, see **3A3g**)(plus 1 for each promoted Bernal), so that you Discard Colo-Remember (Module 2): An nists if the Bernal is destroyed, rather than when it is unanchored. unanchored Home Bernal

- **Dirt Refuel (2B6c)**. You may add a gray Wet Mass Chit for the Bernal and set it to any value per **G1c**. This loses its home *benefits* represents converting its dirt shielding into dirt propellant.[50] (2B3, including profits &

- **Unanchored Home (2B6d)**. If you unanchor your Home Bernal, you lose the benefits of **2B3**. It can no faction privileges) and longer function as a *Bank* **(3A3j)**, and if it holds Aqua, convert them to FTs. Your faction privileges and *faction weapons* (3Aa). weapons are locked. Its *Home Ability* **(2B3c)** and *profits* **(2B3d)** are shutdown. You may not boost to there 3E9a **(2B3e)**.
...
**d. Cancer Hospital/Lab.** Place the red cross chit under this Bernal as a reminder that it is not a combat Remember (Module 2): unit, does not interrupt movement, and always evades the *No Home Bernal* restriction **(3D4c)**. No combat is When boosting or building allowed to or from this Bernal or any Colocated Tokens. the Cancer Hospital, it is

**Example 3E9a** your responsibility to add its red cross chit. If you forget, your opponents can't be held responsible if it gets attacked! 3E9d

Red maverick factory 3E8
...
[49] A mobile factory is much more mobile on the ground than in space. Contrary to intuition, (and contrary to Hollywood), almost anything on a surface is much more mobile than anything in space. In a factory versus rocket combat situation, the mobility advantage lies with the factory, not the rocket! A typical space factory with 100 tonnes of mass is lightweight on a low gravity world. On a size 1 asteroid, it weighs 750N, or 169 lbs. Even on a much bigger world, a buggy can tow it at tens of milligravities in any direction. This compares to a rocket with a net thrust of 6, which can accelerate at 10 milligees, after which it is still moving in essentially one direction. A rocket’s practical maneuverability is so limited that, for targeting purposes, it might as well be stationary during the months of its approach. For one thing, a rocket without wings or atmosphere cannot dodge or swoop. For another thing, rockets are massive, mainly due to propellant (unless it is on its last few tonnes of water, in which it effectively becomes ballistic space junk). This mass and resultant kinetic energy associated with high speeds means that a high thrust course change, the type made by rotating the rocket 90 degrees and firing the main engine, will produce a milliradian course change after an hour. This is with the thrusts typical in the game, higher thrusts are possible but waste precious propellant, and still will make little more than a degree of course change in an hour. Rockets are essentially unidirectional except in the very long term.
...
## 3F. ET Home Bernals & Postwar {: #3f-et-home-bernals-and-postwar}

Easily missed: An ET

### Home Bernal is the 3F1. ET Home Bernals (Module 2)

same as a Home Bernal,

including if unanchored or If during War you are a member of the Independence (), or in a postwar *Space Republic* **(3F5a)** (everyone), then Decommissioned. 3F1gyou may count your Stanford Ring Anchored Bernal with a dirtside hydration of 4 or more as your Home Bernal. This Home Bernal, called an **ET Home Bernal,** is the same as a terrestrial Home Bernal except you cannot boost there from Earth **(2B4e)**. Furthermore, these Abilities and Privileges are not lost in War or Anarchy. Additionally:

Note (Module 2): The

**a. Bank.** Transfer your Aquas (blue beads) to your ET Home Bernal, which is now your *Bank* **(3A3j)**. Stanford Bernal designates **b. Faction Privilege & Faction Weapon Unlocked**, even during War/Anarchy or peace. your Home Bernal or ET **c. Bernal Home Abilities Unlocked**, even during War or Anarchy. Home Bernal. But if your [51]

**d. Profits.** One Aqua per turn. Kalpana is anchored **e. Second Bernal.** Although you cannot boost to an ET Bernal, your second Bernal acquired goes directly to where you want your your ET Home Bernal. Home Bernal to be, you **f. LEO Bypassed.** After you anchor an ET Home Bernal, all actions and operations previously performed in LEO are allowed to swap Stacks (except *boost*) must now be performed at this Bernal's orbit, which becomes the destination for: and Tokens to make the

- **Decommission of Crew (E7c)**,

Stanford into a Kalpana. **• Combat Casualties**. If your Colonist is Decommissioned as a result of Murder/Suicide or combat, it

3F1a

immediately restarts here.
...
Tip (Module 2): The Galilean **g. Building Space Elevators (Module 1).** If your ET Home Bernal is a Lofstrom Loop Microgravity Bernal or moons (Callisto, Ganymede, Elevator Bernal in a lagrange space (including lagrange burns), and there is a single lander burn or single half Europa, & Io) are great lander burn between that Space and a Dirtside, then you may perform an *epic hazard operation* **(1A6)** to places for a loftstrom/GEO build a Space Elevator. space elevator. 3F1g
...
- **War Petition.** During War/Anarchy, building an elevator to your ET Home Bernal in the Earth or Mars Zones requires a *petition* **(3D4c)** in the authority ideology.

- **Token.** Place a Space Elevator Token over the Burn, or over the Space Elevator icon printed on the map if there is one.

**h. Radiation Cycler.** Your Anchored Tourism Cycler allows safe passage through any Colocated or Adjacent Radiation Belts. Your Bernal has a Dirtside on Haumea as shown. Because it can't Anchor at a Hazard Space (such as the ring), it Anchors where shown (which is still Adjacent to its Dirtside). By moving a Freighter to the ring and performing an Epic Hazard Roll, you establish a Space Elevator and place a gray stick where shown. If your Bernal is the Lofstrom Loop Bernal, you don't need the Freighter.

**Terrestrial Space Elevator**

**Example 3E9g** Art by Liftport (www.liftport.com).

Space elevator example 3E9g
...
**h. Dyson Bubble** (Colonist Future, **1D5j**). May prevent your opponent from using Solar-Powered cards during combat against you. The use of this Ability may be *negotiated* **(N)** with your allies, regardless of location. Your ally is under attack by a raygun using Solar-Power. Before the dice are rolled, he asks you for help. You respond that you will block the attacker's sun in return for a bribe. In the interest of providing a realistic experience, please limit these sorts of "real time" negotiations to less than 15 seconds.

Negotiation example 3F2h
...
- **First Player.** The current 1st Player remains the 1st Player for the rest of the game. Five propaganda are flipped to Loyalist, leaving only one Independent. As an Independent, on your Turn you decide to capitulate (a free action), flipping the final propaganda to Loyalist. The War ends and your side loses.

The players stack the Seniority Disks and retrieve their delegates, and then the political assembly and all its remaining stuff are unceremoniously dumped into the box!
...
**a. Space Republic** (Flag = ). If the Independents have won, Module 0 but not Module 3 is used. When the Anarchy event occurs use **K2e**, and the rules for *ET Home Bernals* **(3F1)** remain in effect.[54] You no longer need permission from the 1st Player to prospect Luna. You can Anchor Bernals to Luna.

**b. Space Empire** (Flag = ). If the Loyalists have won, Module 0 is not used, but Module 3 can be used. When the Anarchy event occurs use the *Age of Piracy* variant **(3Ad)**, and ET Home Bernals are not used.

**Tip:** If your enemy can win by space lord, or if you

have lots of vulnerable holdings, it may be best to

truce, defect or capitulate. 3F4g
...
## 3G. Module 3 Endgame {: #3g-module-3-endgame}

**Example 3F4c** Endgame Victory Points (VP)

The game ends when Seniority Disks are exhausted per

| Token VP: | 1 VP per wooden or plastic Token in a player color (on map or assembly). Damaged Factories/Colonies and corrupt delegates do not count. | | | |
|---|---|---|---|---|
| Bonus VP: Depends on the Token’s location: | • Factory Stock Price: 2X if linked to a Space Elevator. • Colony Dome Location: +1 Astrobiology, +2: Submarine, Bernal • Glory Chits • Dirtside Hydration: Dirtside Hydration = VP. Terrestrial Home Bernal = 6 VP. • Fu t u r e Stars | 1X, except 2X glory | 1X | 1X,except stock price = 2 VP regardless of track. |
| Politics VP: Increase the Token VP for all players according to final position of Acting Law: | • Freedom = +1 VP per Factory cube. • Honor = + 1 VP per glory chit. • Unity = +1 VP per Colony dome.+1 VP per heroism chit. • Authority = +1 VP per Claim. • Equality = +1 VP for each uncorrupted delegate. • Individuality = +1 VP per wood/ plastic Token on Hazardous Sites. +1 per bribe in Reserves. | 0X (political assembly dissolved) | 1X | 0X |
| Bribe VP: | 1 VP for each bribe in Reserves | 0X | 1X | 2X |
...
Independence wins the War, and later the game ends in a Space Republic. You score Bernal VP:**Promoted Cancer Hospital** exactly as per the Core Game (M2), except no Token VP for For specific damaged Factories/Colonies. Promoted **Tourism Cycler:** +2 VP per Dirtside for this Bernal.

Easily missed: The *political scoring* (O7) for some of the Ideologies are changed from Core. 3G1

**Summary OF Bernal & Bernal lab abilities for module 3:**Colonists futures

**Bernals:** Antimatter Factory Your Crew and Bernals can fire Antimatter Bombs.New Venus & Footfall (1D5a,f, 3F2g) Use the *dislodge operation* (3D7) to complete

Cancer Hospital

Red Cross Token. No combat is allowed to or from Bernal.

these Futures. Completion allows you to convert any

Immune to *No Home Bernal* restriction. Synodic Comet into a synodic dropstone, in which you do Lofstrom Loop Microgravity Bernal Immune to boost & homesteading restrictions. not need to pay a bribe. With an Epic Hazard Op, may build a Space Elevator over a Dyson Bubble (1D5j, 3F2h) May prevent a player from using Solar-Power cards (half)lander burn. Doubles FINAO costs for all players. during combat. Pharmaceutics Bernal You may fire Beamed Gray Goo (pay a bribe). Space Elevator Bernal Same as Lofstrom Loop. Its stick token at Earth is immune **Summary of dropstone dislodge locations:** to attacks. Synodic Dropstone A dropstone dislodged from a Site with a colored border can reach targets (Sites or Bernals) in a Zone no further

**Summary OF Futures Modified FOR Module 3:** from Sol than the Site dislodged from.

TW thruster futures Jovian Himalia Moonlets A dropstone dislodged from one of these 4 moons can Mini-Black Hole (1D3b, 3F2a) May *dislodge* (3D7a) dropstones from any SIze 1 Site. target a Site/Bernal in the Jupiter Zone. Mass Beam (1D3c, 3F2b) The FV of your guns (3E3b) is their firing rad-hardness minus 1d6.Saturnian Norse Moonlets & Methone A dropstone from one of these 6 moons can Enzmann Starship & Lithiated Ammonia (1D3f,g, 3F2c) Destroy any dropstone (1/ target a Site/Bernal in the Saturn Zone. Turn). Freighter FUTURESUranian Setebos/Sycorax/Prospero Moonlets A dropstone from one of these 3 moons can target a Site/Bernal in the Uranus Zone. Beehive Ark (1D4b, 3F2d) Your Tokens on Size 1 Sites are immune to all combat. Antimatter (1D4f, 3F2e) Gain *amat bombs* (3E3d), which you can use without Frostline Dislodge A dropstone dislodged as a 5FT tank of water from a Space paying Aquas. craft can target Site that is Hydration 3 or 4 in the Ceres Golden Apples (1D4e, 3F2f) FINAO is no longer a *war restriction* (3D4i) for all or Jupiter Zones. The Spacecraft must be in the or players. If other players pay for FINAO (whether Aqua or Zones, but not on a Site, and must expend either 5 FTs or 5 Isotopes), you receive the payment. tanks of water fuel in the dislodge operation.

**Bernal**: +1 VP per Colony dome. **Promoted Climate Control Bernal:** +2 VP per Dirtside for this Bernal. **Sso Diplomatic:** +1 VP per delegate in the Ideology of your player color. **Promoted Diplomatic Bernal:** Your delegates are +1 VP each.

## Notes {: #notes}

Copyright © 2020, Ion Game Design & Sierra Madre Games **Authors**: Phil Eklund & Justin Grey **Rules**: Paweł Garycki, Geoff Speare, Dom Rougier, Stefano Tiné Product no: SMG28-4.3
